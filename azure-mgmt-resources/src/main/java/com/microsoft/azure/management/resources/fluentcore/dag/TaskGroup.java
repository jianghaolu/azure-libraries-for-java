        ...
        return Observable.defer(new Func0<Observable<Indexable>>() {
            ...
            public Observable<Indexable> call() {
                if (isGroupCancelled.get()) {
                    ...
                    Function<Indexable, Observable<Indexable>> onResult = Observable::just;
                    Function<Throwable, Observable<Indexable>> onError = taskError -> processFaultedTaskAsync(entry, taskError, context);
                    Callable<Observable<Indexable>> onComplete = () -> processCompletedTaskAsync(entry, context);
    ...
    private Observable<Indexable> invokeAfterPostRunAsync(final TaskGroupEntry<TaskItem> entry,
        ...
        return Observable.defer(new Func0<Observable<Indexable>>() {
            ...
            public Observable<Indexable> call() {
                ...
                Function<Throwable, Observable<Indexable>> onError = error -> processFaultedTaskAsync(entry, error, context);
                Callable<Observable<Indexable>> onComplete = () -> {
                    if (isFaulted) {
                ...
                return postRunObservable.flatMap(i -> null, // no onNext call as stream is created from Completable.
                        onError,
                        onComplete);
            }
        });
